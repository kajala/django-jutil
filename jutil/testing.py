...
class TestSetupMixin:
    @staticmethod
    def add_test_user(email: str = '', password: str = '', **kwargs) -> User:  # nosec
        """
        ...
        """
        if not email:
            email = '{}@example.com'.format(uuid1().hex)
        if not password:
            email = email.split('@')[0]
        user = User.objects.create(username=email, email=email, **kwargs)
